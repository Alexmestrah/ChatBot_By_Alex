...
class Training:
    def __init__(self):
        ...
        self.data=json.loads(data_file)['intents']
        self.ignore_words=list("!@#$%^&*?")
        self.process_data()

    def process_data(self):
        #fetch patterns and tokenize them into words
        self.pattern=list(map(lambda x:x["patterns"],self.data))
        ...
        self.classes= flatten( [[x["tag"]]*len(y) for x,y in zip(self.data,self.pattern)])
